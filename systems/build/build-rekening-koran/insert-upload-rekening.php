<?php
	if (isset($_POST['submit']))
	{
		...
		die ("Mohon dicek ulang file uploadnya <a href='../../dash.php?hp=upload-rekening'><---Kembali---></a>");
		...
		{	
			...
			$id_bank = $data->val($i, 3);
			$nominal = $data->val($i, 4);
			$keterangan = $data->val($i, 5);
			$status = $data->val($i, 6);
			$tanggal_input = $data->val($i, 7);
			...
			$ip2= $hasil2['ip']; */
				...
				$query = "INSERT INTO `tb_upload_rekening` (
						`tanggal` ,
						`id_bank` ,
						`nominal` ,
						`keterangan` ,
				...
				$hasil = mysql_query($query) or die("Gagal Upload Rekening ".mysql_error()."<br/>");
		...
		echo "Total Update Data Gagal Upload : ".$update_gagal."<br><a href='../../dash.php?hp=upload-rekening'><---Kembali---></a>";
	}
?>